...
function transform_vigenere(to: string): void {
    ...
    const decrypt = args.getString("decrypt")!.toString() == "true";


    let crypted = "";
    if(decrypt){
        crypted = decrypt_vigenere(text, key);
    }else{
        crypted = encrypt_vigenere(text, key);
    }

    let string = "";

    let split = crypted.split("\n");
...
function decrypt_vigenere(s: string, key: string): string {
    let result = "";
    let nonNumerical = 0;
    for (let i = 0; i < s.length; i++) {
        let c = s.charCodeAt(i);
        ...
        if (isAlphabetic(s.charAt(i))) {
            ...
            let decryptedCharCode = (c - stringOffset - (k - keyOffset) + 26) % 26 + stringOffset;
...
function manifest(): void {
    Console.log('{"name": "vigenere" ,"version":"1.1.0","description":"this package support vigenere modules which can be used to encrypt text","transforms":[{"from":"vigenere","to":["html","latex"],"arguments":[{"name":"key","default":"key","description":"key used to decrypt text", "type": "string"},{"name":"decrypt", "default": "false", "description": "set to true to decrypt instead of encrypt", "type": ["true", "false"]}]}]}');
    proc_exit(0);
}